...
아니 근데 또 타임아웃 빼고 생각해보면 음... 비슷했던거 같은데
아니 모르겠네 ㄹㅇ
...
핵심 포인트
1. DB 작업이 병목이 되는 상황에서:
    - MVC는 수용 가능한 만큼만 받아들여 실패율이 낮음
    - WebFlux는 더 많은 요청을 받아들이지만 높은 실패율

2. 실제 처리량:
    - 최종적으로 성공적으로 처리하는 양은 MVC가 조금 더 나음
    - 더 많은 요청을 받는 것이 반드시 이점이 되지 않음을 보여줌
