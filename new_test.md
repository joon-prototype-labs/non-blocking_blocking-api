...
HikariCP + Netty 조합에 문제가 있을 거 같음. 일단 HikariCP는 커넥션 객체를 풀로 관리하는거지 스레드를 관리하는게 아님.

따라서 JDBC는 Blocking Call을 사용하므로 Netty의 이벤트 루프 내부에서 호출하면 영향을 받을 거 같음.

그걸 해결하려면 코루틴에서는 별도 DispatcherIO 나 아니면 별도 스레드에서 처리해야하는데, 그러지 않는 상황에서는 문제가 생길거 같고.
